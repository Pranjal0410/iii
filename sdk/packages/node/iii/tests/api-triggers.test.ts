...
import { http, type ApiResponse, type StreamRequest, type StreamResponse } from '../src'
...
describe('API Triggers', () => {
  it('should register GET endpoint', async () => {
    const fn = iii.registerFunction(
      'test.api.get',
      async (_req: StreamRequest): Promise<ApiResponse> => ({
        status_code: 200,
  ...
  it('should register POST endpoint with body', async () => {
    const fn = iii.registerFunction(
      'test.api.post',
      async (req: StreamRequest): Promise<ApiResponse> => {
  ...
  it('should expose raw JSON request body through request_body', async () => {
    ...
    const fn = iii.registerFunction(
      'test::api::json::raw',
      http(async (req: StreamRequest, response: StreamResponse) => {
  ...
  it('should return custom status code', async () => {
    const fn = iii.registerFunction(
      'test.api.notfound',
      async (_req: StreamRequest): Promise<ApiResponse<404>> => ({
        status_code: 404,
  ...
  it('should download a PDF file via streaming response', async () => {
    ...
    const fn = iii.registerFunction(
      ...
      http(async (_req: StreamRequest, response: StreamResponse) => {
