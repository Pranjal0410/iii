...
|---|---|---|---|---|
| **Rust** | `\|input: Value, metadata: Option<Value>\|` — 2-arg closures accepted by `RegisterFunction::new` / `new_async` | `TriggerRequest { … }.metadata(json!(…))` → `TriggerRequestWithMetadata` (`protocol.rs`) | `protocol.rs` `metadata: Option<Value>` | serde `default` + `skip_serializing_if = "Option::is_none"`; 1-arg closures still accepted; public `RemoteFunctionHandler` alias keeps its 1-arg shape (the 2-arg dispatch shape is the new `RemoteFunctionHandlerWithMetadata`); `TriggerTypeRef::register_trigger_with_metadata` retained |
...
| **Python** | `def handler(data, metadata=None)` or `def handler(data, *, metadata=None)` — detected by `_metadata_passing_mode` (`iii.py`) | `trigger({ …, "metadata": … })` — `TriggerRequest.metadata` (`iii_types.py:241`) | `InvokeFunctionMessage.metadata` (`iii_types.py:252`) | metadata forwarded only to handlers declaring a `metadata` parameter; Pydantic `exclude_none` |
| **Go** | `iii.MetadataFromContext(ctx) (json.RawMessage, bool)` — metadata rides on the handler's `ctx` (`metadata.go`) | `TriggerRequest.Metadata` (`client.go`) | `InvokeFunctionMessage.Metadata` (`protocol.go`) | struct field `json:"…,omitempty"`; `nil` omitted; `Handler` / `TypedHandler` signatures and all registration functions unchanged — no new registration API |

**Python back-compat detail:** `_metadata_passing_mode` inspects the user handler's signature and returns `positional` (`def f(data, metadata=None)`), `keyword` (`def f(data, *, metadata=None)`), or `none` (any signature without an explicit `metadata` parameter). Only handlers that declare `metadata` receive it, so pre-existing handlers — including ones with unrelated optional second parameters, `*args`, or `**kwargs` — keep their exact old call shape.
