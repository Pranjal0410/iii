...
async fn vm_writer_task(
    mut vm_write: tokio::net::unix::OwnedWriteHalf,
    mut rx: mpsc::Receiver<FrameBytes>,
) {
    while let Some(frame) = rx.recv().await {
        if let Err(e) = vm_write.write_all(&frame).await {
...
async fn vm_reader_task(mut vm_read: tokio::net::unix::OwnedReadHalf, routes: Routes) {
    loop {
...
async fn listener_task(
    ...
    vm_write_tx: mpsc::Sender<FrameBytes>,
    routes: Routes,
    slots: Slots,
) {
...
async fn client_session(
    ...
    let reader_fut = async move {
        loop {
            ...
            if vm_write_tx_for_reader.send(frame).await.is_err() {
                break;
            }
        }
    };
    ...
    for &id in &ids {
        ...
        // `.send().await` on the bounded channel. Blocking briefly on
        // a saturated queue is the right tradeoff: dropping a SIGKILL
        // would leave the guest session alive past the slot-cooldown
        // window, which is exactly what the cooldown is designed to
        // prevent. If the VM writer is gone the channel's closed and
        // this resolves immediately with an error we ignore.
        let _ = vm_write_tx.send(frame).await;
    }
...
async fn read_frame<R: AsyncReadExt + Unpin>(
    ...
    let mut buf = vec![0u8; 4 + frame_len];
...
mod tests {
    ...
    async fn boot() -> (PathBuf, StdUS, TempDir) {
        ...
        // Tiny pause to let listener tasks bind and get to accept.
    ...
    async fn owned_ids_capped_per_client() {
        ...
        // ceiling. Wait for the relay to process them.
        let drain = 64u32;
        for i in 0..drain {
