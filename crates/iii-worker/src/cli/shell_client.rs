...
async fn run(args: ExecArgs) -> anyhow::Result<i32> {
    ...
    run_with_socket(args, sock).await
}

/// Split from `run` so the --timeout test can drive the whole
/// session lifecycle against a tempdir socket instead of the
/// real `~/.iii/managed/<name>/shell.sock` path.
async fn run_with_socket(mut args: ExecArgs, sock: PathBuf) -> anyhow::Result<i32> {
...
/// State machine for the double-Ctrl-C escalation in `pump_sigint_pipe`.
/// Returns `(signal_to_send, should_exit_after)`.
///
/// First press (count == 1): forward SIGINT, keep pumping — give the
/// guest a chance to shut down gracefully. Second or subsequent press:
/// escalate to SIGKILL and tell the caller to exit with
/// [`EXIT_INTERRUPTED`]. Extracted so the state machine is testable
/// without an actual signal handler.
fn next_signal_on_sigint(count: u32) -> (i32, bool) {
    if count <= 1 {
        (SIG_INT, false)
    } else {
        (SIG_KILL, true)
...
mod tests {
    ...
    /// P1 regression for the --timeout kill switch.
    ///
    /// Boots a minimal Unix-socket mock relay that performs the
    /// 4-byte handshake, accepts the client's Request frame, and
    /// then does nothing. The real `run_with_socket` then races
    /// `read_one_frame` against the 50ms deadline, breaks with
    /// `EXIT_TIMEOUT` (124), and sends `Signal { signal: 9 }` to the
    /// relay as a best-effort kill of the guest session.
    ///
    /// Asserts:
    ///   - exit code is 124 (matches coreutils `timeout(1)`)
    ///   - a SIGKILL frame reaches the mock relay on the wire
    #[tokio::test]
    async fn timeout_elapsed_sends_sigkill_and_exits_124() {
        use iii_supervisor::shell_protocol::decode_frame_body;
        use tempfile::TempDir;
        use tokio::net::UnixListener;

        let tmp = TempDir::new().unwrap();
        let sock = tmp.path().join("shell.sock");
        let listener = UnixListener::bind(&sock).unwrap();

        let (saw_kill_tx, saw_kill_rx) = tokio::sync::oneshot::channel::<()>();
        ...
        tokio::spawn(async move {
            ...
            // Handshake: 4-byte BE id_offset. Picking 0 hands the
            // client a corr_id of 1.
            if stream.write_all(&0u32.to_be_bytes()).await.is_err() {
                return;
            }
            // Read and decode frames until we see Signal{9}.
            let mut saw_kill_tx = Some(saw_kill_tx);
            loop {
                let mut len_buf = [0u8; 4];
                ...
                if let Ok((_, _, msg)) = decode_frame_body(&body)
                    && matches!(msg, ShellMessage::Signal { signal: 9 })
                    && let Some(tx) = saw_kill_tx.take()
                {
                    let _ = tx.send(());
                }
            }
        ...
        let args = ExecArgs {
            name: "unused-in-test".to_string(),
            env: vec![],
            workdir: None,
            tty: false,
            // Block the auto-TTY detection so the test stays in pipe
            // mode regardless of how `cargo test` is invoked.
            no_tty: true,
            timeout: Some("50ms".to_string()),
            command: vec!["/bin/sleep".to_string(), "30".to_string()],
        };
        let code = run_with_socket(args, sock.clone())
            .await
            .expect("run_with_socket Ok");
        assert_eq!(
            code, 124,
            "--timeout elapse must exit with 124 (coreutils convention)"
        );

        // The SIGKILL frame is posted to the writer just before the
        // main task returns, so give it a small window to flush to
        // the mock relay's socket.
        tokio::time::timeout(std::time::Duration::from_secs(2), saw_kill_rx)
            .await
            .expect("timed out waiting for SIGKILL frame on the wire")
            .expect("SIGKILL oneshot sender was dropped without firing");
    }

    /// P1 regression for the double-Ctrl-C escalation in
    /// `pump_sigint_pipe`. Covers the state machine extracted as
    /// `next_signal_on_sigint`: first press is a polite SIGINT, second
    /// press escalates to SIGKILL and instructs the caller to exit
    /// with EXIT_INTERRUPTED (128 + SIGINT = 130).
    #[test]
    fn two_ctrl_c_escalates_to_sigkill_and_exits_130() {
        assert_eq!(next_signal_on_sigint(1), (SIG_INT, false));
        assert_eq!(next_signal_on_sigint(2), (SIG_KILL, true));
        // Further presses stay in the kill-and-exit branch — the
        // caller would have exited on press #2, but belt + suspenders.
        assert_eq!(next_signal_on_sigint(3), (SIG_KILL, true));
        assert_eq!(
            EXIT_INTERRUPTED, 130,
            "EXIT_INTERRUPTED must match the shell `128 + SIGINT` convention"
        );
    }
}
