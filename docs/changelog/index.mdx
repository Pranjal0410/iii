...
<Update label="0.20.0" description="June 2026">
  ...
  - Removed: `UpdateBuilder`, the Rust `Value` re-export, `IIIForbiddenError` and `IIITimeoutError` (Python), the `TriggerActionType` alias, `FieldPath` and `MergePath` (Rust path helpers), and the root `Logger` and OpenTelemetry re-exports (use `@iii-dev/observability`).
