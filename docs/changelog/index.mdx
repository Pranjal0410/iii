...
<Update label="0.18.0" description="June 2026">
  ## Type declarations moved to a `types` submodule

  **Breaking.** A set of type/data declarations is no longer exported from the package root. Import
  them from the SDK's `types` submodule instead:

  | Symbol | SDK(s) | Before | After |
  |---|---|---|---|
  | `HttpMethod` | Rust | `use iii_sdk::HttpMethod;` | `use iii_sdk::types::HttpMethod;` |
  | `FunctionInfo`, `TriggerInfo`, `WorkerInfo`, `WorkerMetadata` | Rust | `use iii_sdk::WorkerInfo;` | `use iii_sdk::types::WorkerInfo;` |
  | `IIIConnectionState` | Rust | `use iii_sdk::IIIConnectionState;` | `use iii_sdk::types::IIIConnectionState;` |
  | `ApiRequest`, `ApiResponse` | Rust | `use iii_sdk::ApiRequest;` | `use iii_sdk::types::ApiRequest;` |
  | `AuthInput`, `AuthResult` | Rust | `use iii_sdk::AuthInput;` | `use iii_sdk::types::AuthInput;` |
  | `MessageType` | Python | `from iii import MessageType` | `from iii.types import MessageType` |
  | `ApiRequest`, `ApiResponse`, `HttpRequest`, `HttpResponse`, `AuthInput`, `AuthResult` | Python | `from iii import ApiRequest` | `from iii.types import ApiRequest` |
  | `MessageType` | Node | `import { MessageType } from 'iii-sdk'` | `import type { MessageType } from 'iii-sdk/types'` |
  | `ApiRequest`, `ApiResponse`, `HttpRequest`, `HttpResponse`, `AuthInput`, `AuthResult` | Node | `import type { ApiRequest } from 'iii-sdk'` | `import type { ApiRequest } from 'iii-sdk/types'` |

  In Python and Node, `IIIConnectionState` is now also reachable from the same `types` submodule
  (`from iii.types import IIIConnectionState` / `import type { IIIConnectionState } from 'iii-sdk/types'`).
  `HttpRequest` / `HttpResponse` move in Python and Node only (the Rust SDK has no such types).
  The root continues to export the runtime API — `register_worker` / `registerWorker`, the client
  handle, triggers, channels, and errors — unchanged.

  See [the migration entry](./0-18-0/move-types-to-types-submodule) for the full before/after diff per SDK.
</Update>
