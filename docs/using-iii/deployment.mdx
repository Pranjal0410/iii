...
application is made of workers, which come from two places:

- **Built-in workers** (`iii-http`, `iii-state`, `iii-queue`, `iii-cron`, `iii-stream`,
  `iii-observability`, ...) run in-process inside the engine. You enable them by listing them in
  `config.yaml`.
- **Your own workers** (Node, Python, Rust) are separate processes that connect to the engine over
  WebSocket. The engine image is distroless and carries no language runtime, so it cannot host them.
  Deploy each as its own container and point `III_URL` at the engine
  (`ws://iii:49134` on the compose network). See
...
Point adapters at the mounted path, for example `file_path: /data/state_store.db`. The image runs as
...
The engine is stateless. State lives in workers, and each stateful worker selects a storage
...
File-based storage is the simplest durable option: no extra services, just a writable path on a
...
<Note>
  ...
  to a remote adapter after first boot requires updating the persisted configuration entry, not just
  the `config.yaml` block (see the note above).
</Note>
...
const iii = registerWorker(process.env.III_URL); // ws://iii:49134
...
workers:
  - name: iii-worker-manager
    config:
      host: 0.0.0.0
      port: 49334            # main, trusted listener

  - name: iii-worker-manager
    config:
      host: 0.0.0.0
      port: 49335            # public RBAC listener
      rbac:
...
- `memory` (default) — keeps recent telemetry in the engine for query through its observability
  functions. Bounded by `memory_max_spans` and retention settings.
