...
Build a custom Rust worker that extends the iii engine with new functions, triggers, and adapter integrations.

## Context

Use this guide when you need to add capabilities to the engine that aren't covered by built-in workers. Custom workers are written in Rust and compiled into the engine. For simpler extensions using SDK-based workers, see [Functions and Triggers](/how-to/use-functions-and-triggers).
