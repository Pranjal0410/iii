...
### Bind functions to the custom trigger
...
Before creating a custom type, check the [built-in trigger types](/how-to/use-functions-and-triggers#trigger-types) — `http`, `cron`, `durable:subscriber`, `subscribe`, `state`, and `stream` cover the most common event sources.
</Info>
