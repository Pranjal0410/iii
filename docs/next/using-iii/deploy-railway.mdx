...
- Bundle workers dispatched through the same libkrun rails. (Some workers are
  moving to `deploy: binary`, which **does** run on Railway as a plain process.
  Check the worker's current type before assuming.)
